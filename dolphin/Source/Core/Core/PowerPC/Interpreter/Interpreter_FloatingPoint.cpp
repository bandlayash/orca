                      ...
                      RoundingMode rounding_mode)
{
  ...
  case RoundingMode::TowardsNegativeInfinity:
    ...
    break;
  default:
    // All PowerPC rounding modes (0-3) are handled above.
    // This branch is unreachable in normal execution, but WASM compiles
    // std::unreachable() to a runtime trap, so fall back to nearest instead.
    rounded = RoundToIntegerMode(b);
    break;
  }
