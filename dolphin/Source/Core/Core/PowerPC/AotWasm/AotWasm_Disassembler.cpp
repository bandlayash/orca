...
s32 AotWasmEmitter::PoisonCallback(std::ostream& stream, const void* operands)
{
  stream << "PoisonCallback()\n";
  return sizeof(AnyCallback);
}

s32 AotWasm::StartProfiledBlock(std::ostream& stream,
...
s32 AotWasm::EndBlock(std::ostream& stream, const EndBlockOperands<profiled>& operands)
{
...
s32 AotWasm::Interpret(std::ostream& stream, const InterpretOperands& operands)
{
...
s32 AotWasm::HLEFunction(std::ostream& stream, const HLEFunctionOperands& operands)
{
...
s32 AotWasm::WriteBrokenBlockNPC(std::ostream& stream,
...
s32 AotWasm::CheckFPU(std::ostream& stream, const CheckHaltOperands& operands)
{
...
s32 AotWasm::CheckBreakpoint(std::ostream& stream, const CheckHaltOperands& operands)
{
  const auto& [power_pc, current_pc, downcount] = operands;
...
s32 AotWasm::CheckIdle(std::ostream& stream, const CheckIdleOperands& operands)
{
...
s32 AotWasm::WasmBlockEntry(std::ostream& stream, const WasmBlockOperands& operands)
{
  fmt::println(stream, "WasmBlockEntry(func_id={}, downcount={}, num_load_stores={}, num_fp_inst={})",
               operands.func_id, operands.downcount, operands.num_load_stores, operands.num_fp_inst);
  return sizeof(AnyCallback) + sizeof(operands);
}

static std::once_flag s_sorted_lookup_flag;

std::size_t AotWasm::Disassemble(const JitBlock& block, std::ostream& stream)
{
...
#define LOOKUP_KV(...) {AnyCallbackCast(__VA_ARGS__), AnyDisassembleCast(__VA_ARGS__)}
  ...
  static auto sorted_lookup = std::to_array<LookupKV>({
      LOOKUP_KV(AotWasm::PoisonCallback),
      LOOKUP_KV(AotWasm::StartProfiledBlock),
      LOOKUP_KV(AotWasm::EndBlock<false>),
      LOOKUP_KV(AotWasm::EndBlock<true>),
      LOOKUP_KV(AotWasm::Interpret<false>),
      LOOKUP_KV(AotWasm::Interpret<true>),
      LOOKUP_KV(AotWasm::InterpretAndCheckExceptions<false>),
      LOOKUP_KV(AotWasm::InterpretAndCheckExceptions<true>),
      LOOKUP_KV(AotWasm::HLEFunction),
      LOOKUP_KV(AotWasm::WriteBrokenBlockNPC),
      LOOKUP_KV(AotWasm::CheckFPU),
      LOOKUP_KV(AotWasm::CheckBreakpoint),
      LOOKUP_KV(AotWasm::CheckIdle),
      LOOKUP_KV(AotWasm::WasmBlockEntry),
  });
