                         ...
                         float far_depth)
{
  ...
  // WebGL2 forbids zNear > zFar.  Dolphin's GL backend uses inverted depth
  // ranges for depth-clipping tricks, so we clamp here and compensate by
  // inverting the depth test direction via glDepthFunc in the pipeline state.
