...
static void InitVersion()
{
  ...
  // WebGL 2.0 does not expose GL_MAJOR_VERSION / GL_MINOR_VERSION via getParameter.
  // Parse the GL_VERSION string instead; WebGL 2.0 reports "OpenGL ES 3.0 (WebGL 2.0 ...)".
  ...
  s_gl_version = 300;  // safe default: WebGL 2.0 = GLES 3.0
#else
  GLint major, minor;
...
bool InitFunctionPointers(GLContext* context)
{
  ...
  {
#ifdef __EMSCRIPTEN__
    // On Emscripten, always attempt to resolve every function pointer regardless of the desktop
    // extension requirements. WebGL 2.0 may expose OES/EXT variants of functions that have
    // different desktop requirement strings (e.g. glDrawElementsBaseVertex via
    // GL_OES_draw_elements_base_vertex vs GL_ARB_draw_elements_base_vertex).
    // Use "first non-null wins": skip entries where the pointer was already resolved, so that
    // a later fallback variant (e.g. glTexBufferEXT resolving to null) doesn't overwrite a
    // successful OES resolution. Missing functions stay null; OGLConfig's bSupports* flags
    // guard actual usage.
    if (*it.function_ptr == nullptr)
